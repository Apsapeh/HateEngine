...
Texture HERResource::asTexture(
    Texture::TexWrap tex_wrap,
    Texture::TexFiltering tex_filtering,
    bool mipmap, float mipmap_bias,
    bool autoload
) {
    ...
    unsigned char *s_data = stbi_load_from_memory(
        this->data.data(), this->data.size(),
        &width, &height, &n, 0
    );
    if (n == 4) textureFormat = Texture::RGBA;
    this->data = std::vector<uint8_t>(
        s_data,  s_data + width * height * n
    );
    stbi_image_free(s_data);
    return Texture(
        this->data, width, height, textureFormat,
        tex_wrap, tex_filtering, mipmap, mipmap_bias
    );
}
